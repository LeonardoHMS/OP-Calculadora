...
class ProgramPainel:
    def __init__(self):
        ...
        _layout = [
            ...
            [
                ...
                sg.CalendarButton(
                    'C',
                    format='%d-%m-%Y',
                    month_names=funcoes.MES,
                    day_abbreviations=funcoes.ABV_DIAS
                ),
                sg.Text('Dia Fim'),
        ...
        self.window = sg.Window(
            ...
            use_default_focus=False).layout(_layout)
    ...
    def run(self):
        while True:
            ...
            if event == sg.WIN_CLOSED or event == 'Sair':
                ...
                break
            try:
                # DataScience para pegar os dados somente das produções
                # que foram finalizadas com valor total
                if event == 'Cabeçalho':
                ...
                # em minutos do tempo de produção
                if event == 'Confirmar':
                    resultados = (funcoes.calcular_horario(
                        self.values['inicio'],
                        self.values['fim'],
                        self.values['operadores'],
                        self.values['parada'].strip(),
                        self.values['d_inicio'],
                        self.values['d_fim']
                    ))  # Função para calcular o horário
                    if resultados:
                        sg.cprint(
                            f'Tempo da Maquina: {resultados[0]:.0f} Minutos.')
                        sg.cprint(
                            f'Tempo de Mão humana: {resultados[1]:.0f} Minutos.'  # noqa:E501
                        )
                    else:
                        sg.cprint('ERRO, Verifique as informações!')
            # Caso de um erro em algum dos campos de dados,
            # uma mensagem será mostrada para o usuário de possiveis erros
            except Exception:
...
class CalculoRefugo():
    ...
    def __init__(self):
        ...
        _layout = [
            [
                sg.Text('Total   '),
                sg.Input(key='total', size=self._size_Input),
                sg.Text('Peso        '),
                sg.Input(key='PesoT', size=self._size_Input)
            ],
            [
                sg.Text('Refugo'),
                sg.Input(key='Trefugos', size=self._size_Input),
                sg.Text('Resultado'),
                sg.Text(key='Prefugos')
            ],
            [
                sg.Button('Confirmar', bind_return_key=True),
                sg.Text('Total Gasto'),
                sg.Text(key='peso_total')
            ]
        ]
