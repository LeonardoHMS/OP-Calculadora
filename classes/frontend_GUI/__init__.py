...
class ProgramPainel:
    def __init__(self):
        ...
        _layout = [
            ...
            [
                sg.Text('Dia Início'),
                sg.Input(key='d_inicio', size=(10, 1),
                         default_text=datetime.today().strftime('%d-%m-%Y')),
                sg.CalendarButton(
                    'C', format='%d-%m-%Y', month_names=funcoes.MES, day_abbreviations=funcoes.ABV_DIAS),
                sg.Text('Dia Fim'),
                sg.Input(key='d_fim', size=(10, 1),
                         default_text=datetime.today().strftime('%d-%m-%Y')),
                sg.CalendarButton(
                    'C', format='%d-%m-%Y', month_names=funcoes.MES, day_abbreviations=funcoes.ABV_DIAS)
            ],
            [
                ...
                sg.Text(f'{"":>12}'), sg.Image(r'static/githublogo.png',
                                               key='link', tooltip='acessar', enable_events=True)
            ],
            [
                sg.Text('Oprs.'),
                ...
                sg.Text(f'{"By: LEONARDOHMS"}', enable_events=True,
                        text_color=('black'), font='Arial, 10')
            ],
            [sg.Button('Confirmar', bind_return_key=True),
             sg.Button('Limpar')],
            [sg.Multiline(size=(35, 15), key='__Output__',
                          font=_font_str, do_not_clear=False, disabled=True)]
        ]
        # Janela
        self.window = sg.Window('OP Calculator v2.52', icon=r'static/calculator.ico',
                                return_keyboard_events=True, use_default_focus=False).layout(_layout)
            ...
            try:
                ...
                if event == 'Cabeçalho':
                    arquivo = sg.popup_get_file(
                        'Selecione o arquivo', 'Cabeçalho de ordem', icon=r'static/calculator.ico')
                ...
                if event == 'Componentes':
                    arquivo = sg.popup_get_file(
                        'Selecione o arquivo', 'Componentes', icon=r'static/calculator.ico')
                ...
                if event == 'Tempo Operações':
                    arquivo = sg.popup_get_file(
                        'Selecione o arquivo', 'Tempo de operações', icon=r'static/calculator.ico')
                    funcoes.organizar_tempos_prd(arquivo)
                    sg.cprint('Planilha concluída')

                # Escolha o destino para salvar as planilhas
                if event == 'Definições':
                    destino = sg.popup_get_folder(
                        'Salvar planilhas em', 'Local', icon=r'static/calculator.ico', default_path=funcoes.getDiretorio())
                ...
                if event == 'ENTE - Sem planilha':
                    ...
                    sg.popup('Relatório extraído com sucesso!!!',
                             title='Confirmação', icon=r'static/calculator.ico')
                ...
                if event == 'Ajuda':
                    sg.popup(funcoes.text_popup(), title='Ajuda',
                             icon=r'static/calculator.ico')

                if event == 'Desenvolvedor':
                    if self.debug == False:
                        teste = sg.popup_yes_no('Deseja ativar o Debug ?',
                                                icon=r'static/calculator.ico',)
                        if teste == 'Yes':
                            self.debug = True
                ...
                if event == 'Confirmar':
                    ...
                    if resultados:
                        sg.cprint(
                            f'Tempo da Maquina: {resultados[0]:.0f} Minutos.')
                        sg.cprint(
                            f'Tempo de Mão humana: {resultados[1]:.0f} Minutos.')
                    else:
...
class LoginSAP():
    def __init__(self):
        ...
        _layout = [
            ...
            [sg.Text('Senha'), sg.Input(
                key='senha', size=_size_Input, password_char='*')],
            [sg.Text('acesso SAP'), sg.Input(key='acessosap',
                                             size=_size_Input, default_text=_acesso)],
            [sg.Button('Confirmar')]
        ]
        self.window = sg.Window(
            'Login do SAP', icon=r'static/calculator.ico').layout(_layout)
    ...
    def RunApp(self):
        ...
        if event == 'Confirmar':
            funcoes.setLoginSAP(
                self.values['login'], self.values['senha'], self.values['acessosap'])
...
class CalculoRefugo():
    ...
    def __init__(self):
        ...
        sg.change_look_and_feel('DarkGrey4')
        _layout = [
            [sg.Text('Total   '), sg.Input(key='total', size=self._size_Input), sg.Text(
                'Peso        '), sg.Input(key='PesoT', size=self._size_Input)],
            [sg.Text('Refugo'), sg.Input(key='Trefugos', size=self._size_Input), sg.Text(
                'Resultado'), sg.Text(key='Prefugos')],
            [sg.Button('Confirmar', bind_return_key=True), sg.Text(
                'Total Gasto'), sg.Text(key='peso_total')]
        ]
        self.window = sg.Window('Calculo de Refugos',
                                icon=r'static/calculator.ico').layout(_layout)
    ...
    def RunApp(self):
        while True:
            ...
            if event == 'Confirmar':
                try:
                    resultado = CalculoRefugo.resultadoRefugos(
                        self.values['Trefugos'], self.values['PesoT'], self.values['total'])
                    self.values['PesoT'] = CalculoRefugo.replacePoint(
                        self.values['PesoT'])
                    peso_total = self.values['PesoT'] + resultado
                    self.window['peso_total'].update(
                        str(f'{peso_total:.3f}').replace('.', ','))
                    self.window['Prefugos'].update(
                        str(f'{resultado:.3f}').replace('.', ','))
                except Exception:
